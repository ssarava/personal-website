...
const animateElements = document.querySelectorAll('.skill-category, .project-card, .experience-item, .stat-item');
...
window.addEventListener('mousemove', (e) => {
    ...
    if (gradient1) gradient1.style.transform = `translate(${moveX}px, ${moveY}px)`;
    if (gradient2) gradient2.style.transform = `translate(${-moveX}px, ${-moveY}px)`;
    if (gradient3) gradient3.style.transform = `translate(${moveX * 0.5}px, ${moveY * 0.5}px)`;
});
...
// Console message
console.log('%c🚀 Portfolio Site Loaded!', 'color: #6366f1; font-size: 20px; font-weight: bold;');
